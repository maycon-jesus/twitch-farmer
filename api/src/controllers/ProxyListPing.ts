...
export class ProxyListPingController extends ControllerBase {
    ...
    async setStreamElementsRedemptionsPing(ip:string, ping: number){
        ...
        if(!proxy){
            await this.dd.database.db('proxy_list_ping').where({ip}).insert({ip,streamElementsRedemptionPing:ping})
        }else{
            await this.dd.database.db('proxy_list_ping').where({ip}).update({ip,streamElementsRedemptionPing:ping})
        }
    }
}